...
MERGED_ES = [
    {
        ...
        "faqs":             [
            ...
            ("¿Se recodifica el vídeo al convertir?",
             "Depende del códec, no de la extensión. H.264, AV1 y MPEG-4 (Xvid) se copian sin recodificar. MPEG-2, WMV, VP6 y H.265/HEVC se recodifican a H.264 con calidad alta (<code>-crf 18</code>), lo que tarda más y no deja un vídeo idéntico bit a bit, aunque la diferencia no se aprecia a simple vista. VP8, VP9, H.263, Theora, FLV1 y DivX 3 también se recodifican, aunque el navegador sepa leerlos, porque DaVinci Resolve no los reproduce bien copiados."),
            ("La vista previa de la página no muestra la imagen de mi AVI o 3GP, ¿ha fallado?",
             "No necesariamente. Si el vídeo es Xvid, VidToFLAC lo copia tal cual al MKV y el navegador no siempre sabe decodificarlo para enseñarlo. El archivo descargado está completo; compruébalo abriéndolo en tu editor o en un reproductor como VLC."),
        ...
        "seo_body": (
            ...
            '      <p>Lo que cambia entre formatos es el vídeo. H.264, AV1 y MPEG-4 (Xvid) se copian sin recodificar, así que la imagen no pierde nada. MPEG-2 —el de los VOB, muchos MPEG y algunos TS—, WMV, VP6 y H.265/HEVC se recodifican a H.264 de alta calidad (<code>-crf 18</code>): tarda más, y el resultado es muy parecido al original pero no idéntico bit a bit. Y hay un tercer grupo que también se recodifica aunque el navegador lo lea sin problema: <strong>VP8, VP9, H.263, Theora, FLV1 (Sorenson) y MS MPEG-4 v3 (DivX 3)</strong>. Copiados tal cual, DaVinci Resolve importa el clip pero con VP8 o VP9 no reproduce el audio, y con los otros cuatro no muestra imagen. Está medido uno a uno en el <a href="/banco-de-pruebas/">banco de pruebas</a>.</p>\n\n'
...
MERGED_EN = [
    {
        ...
        "faqs":             [
            ...
            ("Is the video re-encoded during conversion?",
             "It depends on the codec, not the file extension. H.264, AV1 and MPEG-4 (Xvid) are copied without re-encoding. MPEG-2, WMV, VP6 and H.265/HEVC are re-encoded to high-quality H.264 (<code>-crf 18</code>), which takes longer and does not leave the video bit-for-bit identical, although the difference is not visible to the eye. VP8, VP9, H.263, Theora, FLV1 and DivX 3 are re-encoded as well, even though browsers read them fine, because DaVinci Resolve does not play them properly when they are copied."),
            ("The page preview shows no picture for my AVI or 3GP. Did it fail?",
             "Not necessarily. If the video is Xvid, VidToFLAC copies it into the MKV as it is, and the browser cannot always decode it to display it. The downloaded file is complete; check it by opening it in your editor or in a player such as VLC."),
        ...
        "seo_body": (
            ...
            '      <p>What changes from one format to the next is the video. H.264, AV1 and MPEG-4 (Xvid) are copied without re-encoding, so the picture loses nothing. MPEG-2 (found in VOB files, many MPEG files and some TS files), WMV, VP6 and H.265/HEVC are re-encoded to high-quality H.264 (<code>-crf 18</code>): it takes longer, and the result is very close to the original but not bit-for-bit identical. And a third group is re-encoded too, even though browsers read it without trouble: <strong>VP8, VP9, H.263, Theora, FLV1 (Sorenson) and MS MPEG-4 v3 (DivX 3)</strong>. Copied as they are, DaVinci Resolve imports the clip, but with VP8 or VP9 it plays no audio, and with the other four it shows no picture. Each one is measured in the <a href="/en/test-bench/">test bench</a>.</p>\n\n'
            '      <p>When the video is copied but the browser cannot decode it, as with Xvid, the page preview may show no picture. The downloaded file is complete all the same.</p>'
        ),
