...
MERGED_ES = [
    {
        ...
        "faqs":             [
            ...
            ("¿Se recodifica el vídeo al convertir?",
             "Depende del códec, no de la extensión. H.264, AV1, VP8, VP9 y MPEG-4 (Xvid) se copian sin recodificar. MPEG-2, WMV, VP6 y H.265/HEVC se recodifican a H.264 con calidad alta (<code>-crf 18</code>), lo que tarda más y no deja un vídeo idéntico bit a bit, aunque la diferencia no se aprecia a simple vista. H.263, Theora, FLV1 y DivX 3 también se recodifican, aunque el navegador sepa leerlos, porque DaVinci Resolve los importa sin imagen si llegan copiados."),
...
MERGED_EN = [
    {
        ...
        "faqs":             [
            ...
            ("Is the video re-encoded during conversion?",
             "It depends on the codec, not the file extension. H.264, AV1, VP8, VP9 and MPEG-4 (Xvid) are copied without re-encoding. MPEG-2, WMV, VP6 and H.265/HEVC are re-encoded to high-quality H.264 (<code>-crf 18</code>), which takes longer and does not leave the video bit-for-bit identical, although the difference is not visible to the eye. H.263, Theora, FLV1 and DivX 3 are re-encoded as well, even though browsers read them fine, because DaVinci Resolve imports them with no picture when they are copied."),
            ...
            ("Can I convert the DTS audio in a TS or M2TS file?",
             "Yes. The DTS track is decoded and encoded as FLAC, which is lossless, so the editor receives exactly the sound that was decoded. If the file has several audio tracks, VidToFLAC shows a checkbox per track before converting and keeps the ones you leave ticked, each as a FLAC track."),
        ...
        "seo_body": (
            ...
            '      <p>What changes from one format to the next is the video. H.264, AV1, VP8, VP9 and MPEG-4 (Xvid) are copied without re-encoding, so the picture loses nothing. MPEG-2 (found in VOB files, many MPEG files and some TS files), WMV, VP6 and H.265/HEVC are re-encoded to high-quality H.264 (<code>-crf 18</code>): it takes longer, and the result is very close to the original but not bit-for-bit identical. And a third group is re-encoded too, even though browsers read it without trouble: <strong>H.263, Theora, FLV1 (Sorenson) and MS MPEG-4 v3 (DivX 3)</strong>. Copied as they are, DaVinci Resolve imports the clip with sound but no picture. Each one is measured in the <a href="/en/test-bench/">test bench</a>.</p>\n\n'
